...
class TestInputToMaceChain:
    ...
    def mace_graph(
        ...
        unit_cell = (
            np.eye(spatial_dim) * cell_size
        )  # box as a spatial_dim x spatial_dim array
        ...
        graph_data = AtomicData.from_config(
            graph_config, z_table=z_table, cutoff=radial_cutoff
        )
    ...
    def test_input_to_mace(self, score_network_input, radial_cutoff, mace_graph):
        diffusion_for_multi_scale_molecular_dynamics_graph = input_to_mace(
        ...
        for k in ["edge_index", "node_attrs", "positions", "ptr", "batch", "cell"]:
            assert k in diffusion_for_multi_scale_molecular_dynamics_graph.keys()
            assert (
                diffusion_for_multi_scale_molecular_dynamics_graph[k].size()
                == mace_graph[k].size()
            )
            assert torch.equal(
                diffusion_for_multi_scale_molecular_dynamics_graph[k], mace_graph[k]
            )
...
class TestInputToMaceRandom(TestInputToMaceChain):
    ...
    @pytest.fixture()
    def mace_graph(
        self, basis_vectors, cartesian_positions, spatial_dim, n_atoms, radial_cutoff
    ):
        pbc = np.array([True] * spatial_dim)  # periodic boundary conditions
        atom_type = np.ones(n_atoms) * 14
    ...
    def test_input_to_mace(self, score_network_input, radial_cutoff, mace_graph):
        ...
        expected_displacements = (
            mace_graph["positions"][expected_dst_idx]
            + mace_graph["shifts"]
        ...
        computed_displacements = (
            computed_mace_graph["positions"][computed_dst_idx]
            + computed_mace_graph["shifts"]
            - computed_mace_graph["positions"][computed_src_idx]
        )

        # The edge order can be different between the two graphs
        edge_permutation_indices = find_aligning_permutation(
            expected_displacements, computed_displacements
        )

        torch.testing.assert_close(
            computed_mace_graph["shifts"][edge_permutation_indices],
            mace_graph["shifts"],
        )
        torch.testing.assert_close(
            computed_mace_graph["edge_index"][:, edge_permutation_indices],
...
def scrampled_and_expected_irreps(seed):
    ...
    for irrep_string, repetitions in zip(list_irrep_strings, list_repetitions):
        ...
        list_irreps.extend(
            [
                o3.Irreps(f"{multiplicity}x{irrep_string}")
...
def test_get_normalized_irreps_permutation_indices(scrampled_and_expected_irreps):
    ...
    normalized_irreps, column_permutation_indices = (
        get_normalized_irreps_permutation_indices(scrambled_irreps)
    )
...
class TestPretrainedMace:
    ...
    @patch("torch.load")
    def test_download_pretrained_mace_small(
        self,
        mock_load,
        mock_urlretrieve,
        mock_isfile,
    ...
    def test_download_pretrained_mace_invalid_model_name(self, mock_model_savedir):
        ...
        assert "Model name should be small, medium or large. Got invalid_name" in str(
            e.value
        )
...
class TestReshapes:
    ...
    def test_reshape_from_mace_to_e3nn(
        self, mace_format_tensor, irrep, ell_max, num_channels
    ):
        ...
        for ell in range(ell_max + 1):
            start_idx = ell**2
            end_idx = (ell + 1) ** 2
            expected_values = mace_format_tensor[:, :, start_idx:end_idx].reshape(
    ...
    def test_reshape_from_e3nn_to_mace(
        self, e3nn_format_tensor, irrep, ell_max, num_channels
    ):
        ...
        for ell in range(ell_max + 1):
            ...
            expected_values = e3nn_format_tensor[:, start_idx:end_idx].reshape(
                -1, num_channels, 2 * ell + 1
            )
            assert torch.allclose(
