...
def optimizer():
    ...
    optimizer_parameters = OptimizerParameters(
        name="adam", learning_rate=0.001, weight_decay=1e-6
    )
...
def scheduler_parameters(scheduler_name):
    match scheduler_name:
        case "CosineAnnealingLR":
            parameters = CosineAnnealingLRSchedulerParameters(
                name=scheduler_name, T_max=1000, eta_min=0.001
            )
        case "ReduceLROnPlateau":
            parameters = ReduceLROnPlateauSchedulerParameters(
                name=scheduler_name, factor=0.243, patience=17
            )
        case _:
...
def test_load_scheduler(optimizer, scheduler_parameters):
    _ = load_scheduler_dictionary(
        scheduler_parameters=scheduler_parameters, optimizer=optimizer
    )
