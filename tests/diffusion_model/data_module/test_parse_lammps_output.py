...
def fake_thermo_dataframe(pressure: bool, temperature: bool):
    ...
    fake_data_df = pd.DataFrame(
        np.random.rand(number_of_rows, len(keywords)), columns=keywords
    )
    return fake_data_df
...
def expected_processed_thermo_dataframe(fake_thermo_dataframe: pd.DataFrame):
    processed_thermo_dataframe = pd.DataFrame(fake_thermo_dataframe).rename(
        columns={
    ...
    processed_thermo_dataframe["energy"] = (
        processed_thermo_dataframe["kinetic_energy"]
...
def test_parse_lammps_outputs(
    fake_lammps_yaml, fake_thermo_yaml, expected_processed_thermo_dataframe, tmpdir
):
    ...
    pd.testing.assert_frame_equal(
        ...
        df[expected_processed_thermo_dataframe.columns],
    )
...
def test_parse_lammps_dump(lammps_dump_path, expected_lammps_dump_dataframe):
    ...
    assert set(computed_lammps_dump_dataframe.columns) == set(
        expected_lammps_dump_dataframe
    )
    ...
    for colname in computed_lammps_dump_dataframe.columns:
        computed_values = np.array(
            [
                list_values
                for list_values in computed_lammps_dump_dataframe[colname].values
            ]
        )
        expected_values = np.array(
            [
                list_values
                for list_values in expected_lammps_dump_dataframe[colname].values
            ]
        )
