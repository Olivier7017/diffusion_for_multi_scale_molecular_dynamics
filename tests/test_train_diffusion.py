...
def get_prediction_head_parameters(name: str):
    if name == "mlp":
        head_parameters = dict(
            name="mlp", hidden_dimensions_size=16, n_hidden_dimensions=3
        )
...
def get_config(
    ...
    diffusion_sampling_config = dict(
        noise={"total_time_steps": 10},
...
class TestTrainDiffusion(TestDiffusionDataBase):
    ...
    def all_paths(self, paths, tmpdir, config):
        all_paths = dict(
            data=paths["raw_data_dir"], processed_datadir=paths["processed_data_dir"]
        )
        ...
        for directory_name in ["dataset_working_dir", "output"]:
            ...
            directory = os.path.join(
                tmpdir, f"{directory_name}_{np.random.randint(99999999)}"
            )
            all_paths[directory_name] = directory

        all_paths["config"] = os.path.join(
            tmpdir, f"config_{np.random.randint(99999999)}.yaml"
        )
    ...
    def test_checkpoint_callback(self, args, all_paths, max_epoch):
        ...
        should_test_epoch_number = [
            False,
            True,
        ]  # the 'best' model epoch is ill-defined. Don't test!

        for model_path, regex, test_epoch_number in zip(
            model_paths, regexes, should_test_epoch_number
        ):
    ...
    @pytest.mark.skip(
        reason="This test fails because of some obscure change in the Pytorch-Lightning library. "
    ...
    def test_restart(self, args, all_paths, max_epoch, mocker):
        ...
        interruptor = DelayedInterrupt(
            how_many_epochs_before_interrupt=interruption_epoch
        )
